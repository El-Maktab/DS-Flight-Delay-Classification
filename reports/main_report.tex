...
\subsection*{\textcolor{accentgold}{9.1 Target Distribution}}
...
The first thing the notebook makes clear is that the task is strongly imbalanced. About \textbf{80.5\%} of flights are \code{on\_time}, while \code{minor\_delay}, \code{major\_delay}, and \code{cancelled} make up the much smaller remainder. That matters immediately for modeling. A classifier can get a deceptively high accuracy score (foreshadowing) just by staying close to the majority class, which is exactly why balanced metrics and business-aware costs were needed later.

\subsection*{\textcolor{accentgold}{9.2 Flight Activity Context}}
...
\subsection*{\textcolor{accentgold}{9.3 Delay Mix by Airline and Month}}
...
Raw counts hide too much because \code{on\_time} dominates the dataset, so the notebook switched to within-group proportions. That view is much more useful. Some airlines, especially \code{NK} and \code{F9}, carry a visibly larger share of \code{major\_delay} and \code{cancelled} flights than the stronger carriers. The month view shows a similar pattern: winter months such as \textbf{January, February, and December} are worse, and \textbf{June and July} also show elevated disruption. These plots justified keeping airline and calendar features in the modeling pipeline.

\subsection*{\textcolor{accentgold}{9.4 Delay Rate by Departure Hour}}
...
\subsection*{\textcolor{accentgold}{9.5 Numerical Features vs the Target}}
...
\subsection*{\textcolor{accentgold}{9.6 Correlation Analysis and Redundancy}}
...
\subsection*{\textcolor{accentgold}{9.7 EDA Takeaways That Affected Modeling}}
...
\subsection*{\textcolor{accentgold}{16.1 Current Limitations}}
