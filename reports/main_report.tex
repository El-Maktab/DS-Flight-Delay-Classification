...
We tracked the full modeling workflow in \textbf{MLflow} using a local SQLite backend (\code{mlflow.db}) and file-based artifacts under \code{mlartifacts/}. That gave us one place to compare runs, check parameter settings, and retrieve the saved reports, predictions, and model files later.
...
\begin{itemize}[label=\textcolor{accentblue}{\textbullet}]
    ...
    \item \textbf{Artifacts:} serialized model file when a fitted estimator exists, \code{evaluation\_report.json}, and exported holdout predictions CSV for downstream error analysis.
\end{itemize}

For the winning run \code{final-hist\_gradient\_boosting}, MLflow logged a balanced HistGradientBoostingClassifier with \code{max\_iter=2000}, \code{max\_leaf\_nodes=31}, \code{min\_samples\_leaf=20}, \code{learning\_rate=0.1}, and \code{61} selected features. It also stored the train, cross-validation, and holdout metrics along with two evaluation artifacts: \code{evaluation/evaluation\_report.json} and \code{evaluation/preds\_final-hist\_gradient\_boosting.csv}.

\subsection*{\textcolor{accentgold}{10.2 Experiment Inventory}}

\begin{tcolorbox}[mainbox, title={Tracked MLflow Experiments in the Repository}]
\renewcommand{\arraystretch}{1.35}
\small
\begin{tabularx}{\textwidth}{L{4.1cm} X C{1.1cm}}
    \toprule
    \textbf{Experiment} & \textbf{Purpose} & \textbf{Runs} \\
    \midrule
    \code{flight-delay-baseline} & Initial baseline model runs and first MLflow integration. & 9 \\
    \code{flight-delay-baseline-better-eval} & Main iterative experiment ladder with improved business-aware evaluation. & 36 \\
    \code{flight-delay-baseline-freq-encoding} & Compare earlier feature engineering with frequency encoding. & 2 \\
    \code{flight-delay-cv-test} & Validate cross-validation logging and comparison workflow. & 1 \\
    \code{flight-delay-baseline-smote} & Early imbalance-handling experiments with SMOTE. & 3 \\
    \code{flight-delay-hyperparameter-tuning} & Focused randomized search for the strongest tree-based families. & 5 \\
    \code{flight-delay-feature-selection-ablation} & Test optional mutual-information feature filtering against the full feature matrix. & 8 \\
    \code{flight-delay-feature-agg} & Evaluate richer route/context aggregation features. & 6 \\
    \code{flight-delay-threshold-tuning} & Late-stage probability threshold experiments for business-aware decisions. & 2 \\
    \code{flight-delay-final-model-selection} & Locked final comparison on the untouched holdout set. & 8 \\
    \midrule
    \multicolumn{2}{r}{\textbf{Total tracked runs}} & \textbf{80} \\
    \bottomrule
\end{tabularx}
\end{tcolorbox}
...
\begin{figure}[H]
    ...
    \caption{MLflow comparison page for the final model-selection runs. The figure shows the final candidates side by side with their tracked metrics.}
...
\begin{figure}[H]
    ...
    \caption{MLflow experiment list for the project, including baseline runs, feature ablations, tuning runs, threshold tuning, and the final model-selection experiment.}
...
\begin{figure}[H]
    ...
    \caption{Chart view of the final MLflow runs. We used it to compare balanced accuracy against the supporting business metrics before locking the final choice.}
    \label{fig:mlflow-final-runs-chart}
\end{figure}

\subsection*{\textcolor{accentgold}{10.3 Why MLflow Matters in This Project}}

MLflow mattered because the modeling pipeline changed several times during the project. We tried different feature sets, imbalance strategies, model families, and evaluation rules. Without tracked runs, it would have been too easy to mistake a noisy improvement for a real one, especially when some tuning results looked good in cross-validation and then weakened on holdout.
...
The final evaluation used \textbf{two standard technical metrics} and \textbf{two business-oriented metrics} for every model. On the technical side, we focused on \textbf{balanced accuracy} and \textbf{macro F1}. On the business side, we used a misclassification-cost scheme that penalized missed disruption events more than false alarms: \code{on\_time}=0.5, \code{minor\_delay}=5, \code{major\_delay}=8, and \code{cancelled}=10 whenever the true class was predicted incorrectly.

\subsection*{\textcolor{accentgold}{12.1 Final Model Comparison on the Locked Holdout Set}}

\renewcommand{\arraystretch}{1.3}
\small
\begin{longtable}{L{3.6cm} C{1.35cm} C{1.45cm} C{1.35cm} C{1.7cm} C{1.8cm}}
    \toprule
    \textbf{Model} & \textbf{Accuracy} & \textbf{Bal. Acc.} & \textbf{Macro F1} & \textbf{Avg. Cost} & \textbf{Cost Reduction} \\
    \midrule
    \endhead
    \rowcolor{accentblue!8}
    HistGradientBoosting & \textbf{0.558} & \textbf{0.459} & 0.331 & \textbf{1.002} & \textbf{0.290} \\
    Hierarchical HistGradientBoosting & 0.624 & 0.449 & \textbf{0.353} & 1.010 & 0.282 \\
    Logistic Regression (balanced) & 0.447 & 0.438 & 0.281 & 1.015 & 0.277 \\
    Extra Trees & 0.801 & 0.339 & 0.335 & 1.212 & 0.079 \\
    XGBoost (balanced) & 0.088 & 0.307 & 0.097 & 1.133 & 0.159 \\
    Random Forest & 0.807 & 0.294 & 0.290 & 1.263 & 0.029 \\
    Logistic Regression (unbalanced) & 0.806 & 0.285 & 0.280 & 1.266 & 0.026 \\
    Majority baseline & 0.805 & 0.250 & 0.223 & 1.292 & 0.000 \\
    \bottomrule
\end{longtable}
\normalsize
...
\begin{itemize}[label=\textcolor{accentblue}{\textbullet}]
    \item \textbf{Accuracy alone is misleading.} The majority baseline, unbalanced logistic regression, random forest, and extra trees all exceed 0.80 plain accuracy because the dataset is dominated by \code{on\_time} flights. Their balanced accuracy is much worse, which means they fail minority classes more often than the raw accuracy suggests.
    \item \textbf{HistGradientBoosting won the final comparison.} It achieved the highest holdout balanced accuracy (\textbf{0.459}) and the lowest average misclassification cost (\textbf{1.002}), while also giving the largest cost reduction (\textbf{0.290}) versus the \code{predict-all-on-time} baseline.
    \item \textbf{The hierarchical model was competitive but not best under the primary objective.} It produced the highest macro F1 (\textbf{0.353}) and higher plain accuracy than the winner, but it still trailed the single-stage HistGradientBoosting model on balanced accuracy, which was the locked selection criterion.
\end{itemize}

\subsection*{\textcolor{accentgold}{12.2 Best Model Detailed Performance}}
...
\begin{tabularx}{\textwidth}{L{4.2cm} X}
    ...
    \midrule
    Training rows / holdout rows & 85,154 / 21,290 \\
    Selected feature columns & 61 \\
    Holdout accuracy & 0.558 \\
    Holdout balanced accuracy & \textbf{0.459} \\
    Holdout macro F1 & 0.331 \\
    Holdout weighted F1 & 0.624 \\
    Average misclassification cost & \textbf{1.002} \\
    Cost reduction vs on-time baseline & \textbf{0.290} absolute cost units \\
    Train vs holdout balanced accuracy & 0.623 vs 0.459 (moderate overfitting, but still best generalization among final candidates) \\
    \bottomrule
\end{tabularx}
\end{tcolorbox}
...
In practice, the best model works like a \textbf{triage model}. It is useful when the goal is to catch severe events, especially cancellations, but it pays for that sensitivity with many false alarms on flights that were actually on time.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.98\textwidth]{figures/err_analysis/best_run_error_scorecard.png}
    \caption{Scorecard for the winning run, combining the main holdout metrics with per-class precision, recall, and F1 for the four delay classes.}
...
\begin{figure}[H]
    ...
    \caption{Confusion-count and class-normalized confusion matrices for the winning HistGradientBoosting model.}
    \label{fig:best-run-confusion}
\end{figure}

\subsection*{\textcolor{accentgold}{12.3 Error Analysis and Business Interpretation}}

The winning run made \textbf{9,415 errors} on the 21,290-flight holdout set, which gives an overall error rate of \textbf{44.2\%}. The weakest class by recall was \textbf{major delay} (29.5\%). The strongest minority-event class was \textbf{cancelled} (60.5\%). Most of the mistakes came from \textbf{over-predicting disruption} rather than missing every disruption case entirely. The three most common confusion pairs were:

\begin{itemize}[label=\textcolor{accentblue}{\textbullet}]
    \item \code{on\_time \rightarrow minor\_delay}: 3,101 cases, or \textbf{32.9\%} of all errors
    \item \code{on\_time \rightarrow major\_delay}: 2,270 cases, or \textbf{24.1\%} of all errors
    \item \code{on\_time \rightarrow cancelled}: 1,286 cases, or \textbf{13.7\%} of all errors
\end{itemize}
...
\begin{figure}[H]
    ...
    \caption{Largest confusion pairs for the winning run. Most errors came from flights that were actually on time but were predicted as minor, major, or cancelled disruptions.}
    \label{fig:best-run-confusion-pairs}
\end{figure}

The slice-level analysis pointed to the same weak spots. The model was least reliable on \textbf{severe-weather slices} and on the \textbf{evening departure bank}. The worst large-volume segment was \code{departure\_hour\_bucket\_code = 4} (the evening bank), where the error rate reached \textbf{67.1\%} across \textbf{4,050 flights}. Several severe-weather codes at origin or destination also produced error rates above \textbf{70\%} on slices with at least 150 samples. Weather is still the hardest operating regime for the current feature set.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.9\textwidth]{figures/err_analysis/best_run_error_slices.png}
    \caption{Highest-error large slices for the winning run. The worst segments cluster around severe-weather codes and the evening departure bank.}
    \label{fig:best-run-error-slices}
\end{figure}
