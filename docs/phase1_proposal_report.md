...
| **Citation**           | U.S. Department of Transportation, Bureau of Transportation Statistics. (2015). _Airline On-Time Performance Data_. Kaggle. |
...
Validation was performed via `flight_delay_classification/validation/validate.py` covering 12 checks. Results are reproduced from `notebooks/validation_report.ipynb`.
...
| Flight-level duplicates (AIRLINE + FLIGHT_NUMBER + MONTH + DAY) | 864   | Codeshare flights — multiple airlines operating the same physical flight; expected |
